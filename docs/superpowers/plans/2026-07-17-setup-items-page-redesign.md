...
// Keep the assigned VOCALIST mic mirrored as one auto-managed, checkable item (kind:'mic')
// at the front of the bucket, so it counts toward X/Y and remembers its done-state with zero
// new persistence. Create/update/remove to match v.micAssigned. Band mics are ordinary items.
function syncAssignedMicItem(p) {
  ...
  const existing = bucket.items.find(it => it.kind === 'mic');
  if (!mic) { if (existing) bucket.items = bucket.items.filter(it => it.kind !== 'mic'); return; }
  if (existing) { if (existing.text !== mic) existing.text = mic; return; }
  bucket.items.unshift({ id: 'si_mic_' + Math.random().toString(36).slice(2, 9), text: mic, doneThisService: false, scopeOneTime: false, autoAdded: true, kind: 'mic' });
}
