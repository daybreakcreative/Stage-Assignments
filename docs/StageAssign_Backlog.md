...
- **[DONE] Multi-venue profiles — COMPLETE (all 5 phases).** Carlsbad vs Meadowlark each keep their own room config; this week's roster and global settings are shared.
  - **Model:** a venue = the per-ROOM slice of state. Live `state` reflects the active venue; switching stashes the current room's fields, loads the target's, re-renders. Migration wraps any existing single-config install into a "Carlsbad" venue automatically (`ensureVenues`, runs at top of `init()`).
