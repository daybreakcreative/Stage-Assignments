...
// the REAL data, hides the default #dvLayout, and — critically — switching back to a non-bespoke
// world (molten) re-shows #dvLayout and retires #dvWorldRoot.
...
window.addEventListener('load',()=>setTimeout(()=>{
 ...
 check('switching to molten re-shows #dvLayout and retires #dvWorldRoot', ()=>{
   ev("state.world='molten'; applyWorld(); renderDisplayView();");
   const lay = doc.getElementById('dvLayout');
   const root = doc.getElementById('dvWorldRoot');
   if (!lay || lay.style.display === 'none') throw new Error('#dvLayout not re-shown for molten');
   if (root && root.style.display !== 'none') throw new Error('#dvWorldRoot not hidden for molten');
   if (root && root.innerHTML.trim() !== '') throw new Error('#dvWorldRoot not emptied for molten');
 });
